...
export default function ChapterShell({ chapter, beats, onStep, watch, extras, outro }: Props) {
  ...
  // Quiet next-chapter chip. Sits top-right of the chapter header on desktop;
  // stacks BELOW the title block on small screens so it doesn't squeeze the
  // title into a narrow wrapping column. Lights up on the last beat.
  const nextChip = next ? (
    <Link
      to={next.path}
      ...
      <header className="mb-5 lg:mb-7 flex flex-col md:flex-row md:items-start md:justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <span className="text-xs uppercase tracking-widest text-paper/50">{chapter.group}</span>
          </div>
